...
/// A fully resolved move with origin, destination, and special move data.
///
...
impl NotationMove {
    /// Parses algebraic notation into a NotationMove.
    /// move_index determines turn: even = white (rank 0), odd = black (rank 7).
    pub fn parse(input: &str, move_index: usize) -> Option<NotationMove> {
        ...
        Some(NotationMove { piece, dest, threat, capture, promotion })
    }

    fn parse_castling(clean: &str, rank: u8, threat: Threat, capture: Capture) -> Option<NotationMove> {
        match clean {
            "OO" => Some(NotationMove {
                piece: Piece::King,
...
mod tests {
    ...
    #[test]
    fn move_knight() {
        let m = NotationMove::parse("Nf3", 0).unwrap();
    ...
    #[test]
    fn move_capture() {
        let m = NotationMove::parse("Bxc6", 0).unwrap();
    ...
    #[test]
    fn move_bishop() {
        let m = NotationMove::parse("Bb5", 0).unwrap();
        assert_eq!(m.piece, Piece::Bishop);
    ...
    #[test]
    fn move_queen() {
        let m = NotationMove::parse("Qh5+", 0).unwrap();
    ...
    #[test]
    fn move_king() {
        let m = NotationMove::parse("Kf1", 0).unwrap();
    ...
    fn castling_kingside_white() {
        let m = NotationMove::parse("O-O", 0).unwrap();
        assert_eq!(m.piece, Piece::King);
    ...
    fn castling_kingside_black() {
        let m = NotationMove::parse("O-O", 1).unwrap();
        assert_eq!(m.piece, Piece::King);
    ...
    fn castling_queenside_white() {
        let m = NotationMove::parse("O-O-O", 0).unwrap();
        assert_eq!(m.piece, Piece::King);
    ...
    fn castling_queenside_black() {
        let m = NotationMove::parse("O-O-O", 1).unwrap();
        assert_eq!(m.piece, Piece::King);
    ...
    #[test]
    fn move_rook() {
        let m = NotationMove::parse("Rad1", 0).unwrap();
    ...
    fn move_invalid_file() {
        assert!(NotationMove::parse("Ni4", 0).is_none());
    }

    #[test]
    fn move_invalid_rank() {
        assert!(NotationMove::parse("Ne9", 0).is_none());
        assert!(NotationMove::parse("Ne0", 0).is_none());
    }
    ...
    #[test]
    fn capture_detected() {
        let m = NotationMove::parse("Nxe5", 0).unwrap();
        assert_eq!(m.capture, Capture::Taken);
    }

    #[test]
    fn no_capture_by_default() {
        let m = NotationMove::parse("Ne5", 0).unwrap();
        assert_eq!(m.capture, Capture::None);
    }

    #[test]
    fn capture_with_check() {
        let m = NotationMove::parse("Bxf7+", 0).unwrap();
        assert_eq!(m.piece, Piece::Bishop);
        assert_eq!(m.threat, Threat::Check);
        assert_eq!(m.capture, Capture::Taken);
    }
    ...
    fn checkmate_detected() {
        let m = NotationMove::parse("Qf7#", 0).unwrap();
        assert_eq!(m.piece, Piece::Queen);
    ...
    fn promotion_detected() {
        let m = NotationMove::parse("e8=Q", 0).unwrap();
        assert_eq!(m.piece, Piece::Pawn);
    ...
    fn promotion_with_capture() {
        let m = NotationMove::parse("exd8=Q", 0).unwrap();
        assert_eq!(m.piece, Piece::Pawn);
    ...
    fn promotion_to_knight() {
        let m = NotationMove::parse("e8=N", 0).unwrap();
        assert_eq!(m.promotion, Some(Piece::Knight));
    }

    #[test]
    fn promotion_to_rook() {
        let m = NotationMove::parse("a1=R", 1).unwrap();
